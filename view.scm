...
(define-structure view
    ...
    (window (define (view-window self)
	    ...
	    #f)
    (user->x (lambda (x) (inexact->exact (round x))))
    (user->y (lambda (x) (inexact->exact (round x))))
    (user->lw (lambda (x) (if x (inexact->exact (round x)) 0)))
    (x->user (lambda (x) x))
    (y->user (lambda (x) x))
    (user->width (lambda (x) (inexact->exact (round x))))
    (user->height (lambda (x) (inexact->exact (round x))))
...
(define (transform-a-view view originx originy scalex scaley scalelw)
    ...
    (let ((was-originx (view-originx *current-view*))
	 ...
	 (let ((originx (view-originx *current-view*))
	      ...
	      (set! user->x
		    (lambda (x) (inexact->exact (round (+ (* x scalex) originx)))))
	      (set! user->y
		    (lambda (y) (inexact->exact (round (+ (* y scaley) originy)))))
	      (set! user->lw
		    (lambda (x) (if x (inexact->exact (round (* scalelw x))) 0)))
	      (set! x->user (lambda (x) (/ (- x originx) scalex)))
	      (set! y->user (lambda (y) (/ (- y originy) scaley)))
	      (set! user->width
		    (lambda (w) (inexact->exact (round (abs (* w scalex))))))
	      (set! user->height
		    (lambda (h) (inexact->exact (round (abs (* h scaley))))))
