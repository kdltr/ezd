...
(define (psinit width height)
	...
	*pspaper* *pswidth* *psheight*)
	...
	(inexact->exact
	    (quotient (round (- pswidth (if rotate height-pts width-pts))) 2))
	(inexact->exact
	    (quotient (round (- psheight (if rotate width-pts height-pts))) 2))
	(inexact->exact
	    (- pswidth
	       (quotient (round (- pswidth (if rotate height-pts width-pts))) 2)))
	(inexact->exact
	...
	(pscommand 90 "rotate" 0 (- pswidth) "translate"
	    (inexact->exact (quotient (round (- psheight width-pts)) 2))
	    (inexact->exact (quotient (round (- pswidth height-pts)) 2))
	    "translate" "% INCLUDE-DELETE")
	(pscommand 
	    (inexact->exact (quotient (round (- pswidth width-pts)) 2))
	    (inexact->exact (quotient (round (- psheight height-pts)) 2))
	    "translate" "% INCLUDE-DELETE"))
    (pscommand
